...
from fs_grl.data.episode.episode import Episode, EpisodeHParams, MolecularEpisode
...
class CosineEpisodeBatch(EpisodeBatch):
    ...
    def get_cosine_targets(cls, episode_list: List[Episode]) -> torch.Tensor:
        ...
        for episode in episode_list:
            ...
            for query, label in itertools.product(episode.queries, episode.property):
...
class MolecularEpisodeBatch:
    def __init__(
        ...
        :param supports: supports for all the episodes in the batch (Bx2xK)
        :param queries: queries for all the episodes in the batch (Bx2xQ)
        :param properties: tensor containing for each episode the considered properties (B)
        :param episode_hparams: K, Q
        :param num_episodes: how many episodes per batch, i.e. batch size
        """
        ...
        self.num_samples_per_episode = {
            ...
            SampleType.SUPPORT: self.episode_hparams.num_supports_per_class * 2,
        }
        self.samples = {SampleType.QUERY: self.queries, SampleType.SUPPORT: self.supports}

    @classmethod
    def episode_batch_kwargs(cls, episode_list: List[MolecularEpisode], episode_hparams: EpisodeHParams) -> Dict:
        """
        Returns the arguments for the EpisodeBatch constructor

        :param episode_list: list of episodes
        :param episode_hparams: K and Q
    ...
    def from_episode_list(
        cls,
        episode_list: List[MolecularEpisode],
        episode_hparams: EpisodeHParams,
    ) -> "MolecularEpisodeBatch":
        """
        Constructs a new EpisodeBatch.
        ...
        :return: new MolecularEpisodeBatch
        """
    ...
    def split_in_episodes(self, sample_type: SampleType):
        ...
        EpisodeBatch contains Bx2xK support graphs and Bx2xQ query graphs

        Returns B lists of (2xK) graphs each if sample_type is support
            or  B lists of (2xQ) graphs each if sample_type is query
        """

        samples_batch = self.samples[sample_type]

        # list containing (BxK) supports or (BxQ) queries
        samples_list = Batch.to_data_list(samples_batch)
