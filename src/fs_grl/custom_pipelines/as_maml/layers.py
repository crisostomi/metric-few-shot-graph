...
from torch_geometric.utils import add_self_loops, dense_to_sparse, remove_self_loops, softmax
from torch_scatter import scatter_add
from torch_sparse import coalesce, spspmm

from fs_grl.custom_pipelines.as_maml.sparse_softmax import Sparsemax
...
class HGPSLPoolFw(torch.nn.Module):
    def __init__(
        self,
        in_channels,
        ratio=0.8,
        ...
        super(HGPSLPoolFw, self).__init__()
        self.in_channels = in_channels
        self.ratio = ratio
        self.sample = sample
        self.sparse = sparse
...
class LinearFw(nn.Linear):  # used in MAML to forward input with fast weight
    def __init__(self, in_features, out_features):
        super(LinearFw, self).__init__(in_features, out_features, bias=True)
